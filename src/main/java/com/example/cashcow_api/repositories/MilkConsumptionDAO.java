...
public interface MilkConsumptionDAO extends JpaRepository<EMilkConsumption, Integer>, JpaSpecificationExecutor<EMilkConsumption>{
    ...
    @Query(
        value = "SELECT new com.example.cashcow_api.dtos.milk.MilkConsumptionSummaryDTO(c.litrePrice, cc.name, cast(c.createdOn as LocalDate), c.quantity, s.name, (c.litrePrice * c.quantity) ) "
            ...
            + "GROUP BY cast(c.createdOn as LocalDate), cc.name, s.name, c.litrePrice, c.quantity "
